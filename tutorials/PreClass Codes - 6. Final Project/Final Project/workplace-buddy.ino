...
#define WIFI_PUBLISH_INTERVAL 15*1000; // Thingspeak requires inimum 15s interval
...
void loop() {
  ...
  if (wifiPublishTime < now && wifiStatus == WL_CONNECTED) {
    HTTPClient http;
    sprintf(tsUrl, "https://api.thingspeak.com/update?api_key=THINGSPEAK_APIKEY&field1=%d&field2=%d&field3=%d&field4=%d", c, h, bars, isUserPresent);
    http.begin(tsUrl);
