...
sudo docker run -dt --rm --network iot --name influxdb -v ~/influxdb:/var/lib/influxdb influxdb
...
sudo docker run -dt --rm --network iot --name nodered -p 1880:1880 nodered/node-red-docker
